# ---------------------------------------------------------------
#               RS_Framed_kmers Pipeline (Based on RiboDoc)
# ---------------------------------------------------------------
# Imports
...
ribodoc_kmerIndex_tools = config["paths"]["ribodoc_kmerIndex_tools"]
...
onsuccess:
    ...
    shell("rm -f -r " + results_path + "qualitativeAnalysis/bedCount/ " + results_path + "qualitativeAnalysis/*tempoR.Rout")
...
rule all:
    input:
        ...
        expand(results_path + "kmerCount/Kmer/{sample}.tsv", sample=SAMPLES),
...
rule find_adapter_sequence:
    ...
    shell:
        ...
        "Rscript " + ribodoc_kmerIndex_tools + "others/find_adapter_sequencearg.R {input.fastq} {params.Result_path} 2> {log.rscript} ;"
...
rule filter_by_5UTR:
    ...
    shell:
        "bash " + ribodoc_kmerIndex_tools + "others/filter_5UTR.sh -g {input.gff_namedCDS} -o {output.gff_5UTR_filtered} -p {params.annex_path} -r " + config["gff_mRNA_feature"] + " -u " + config["gff_5UTR_feature"] + " -t " + utr_threshold + " 2> {log}"
...
rule ORFget:
    ...
    shell:
        "python3 " + ribodoc_kmerIndex_tools + "orfget/ORFget.py -fna {input.fasta} -gff {input.gff} -features_include " + config['gff_cds_feature'] + " -name_attribute " + config['gff_name_attribut'] + " -o {params.path} -type nucl -elongate 50 -check 2> {log.orf_get}"
# ---------------------------------------------------------------
...
rule riboWaltz_transcriptome:
    ...
    output:
        psite_table = results_path + "riboWaltz" + frag_length_L + "/psite_offset.csv",
        psite_length = results_path + "riboWaltz" + frag_length_L + "/frame_psite_length.csv",
        #metaprofile = results_path + "riboWaltz" + frag_length_L + "/transcriptome_elongated." + SAMPLES[0] + "/metaprofile_psite_-" + config['window_utr'] + "+" + config['window_cds'] + ".tiff"
    log:
    ...
    shell:
        ...
        "Rscript " + ribodoc_kmerIndex_tools + "ribowaltz/periodicity_riboWaltz_transcriptome_3.R -w " + local_path + " -f " + ribodoc_kmerIndex_tools + "ribowaltz/ -g {input.transcriptome_gtf} --path_prefix " + results_path + " 2> {log} ;"
...
rule create_phase_table:
    ...
    shell:
        (
            "python3 " + ribodoc_kmerIndex_tools + "ribowaltz/filter_merge_for_KmerCount.py "
            "{input.psite_length} {input.psite_table} {output.psite_table_offset} {output.psite_table_for_KmerCount}"
        )
# ---------------------------------------------------------------
#               Rule: K-mer Creation
# ---------------------------------------------------------------
rule creationKmer:
    ...
    shell:
        """
        python3 {ribodoc_kmerIndex_tools}kmers_index/processCountingKmer.py \
...
rule suppressionLettre:
    ...
    shell:
        (
            "python3 " + ribodoc_kmerIndex_tools + "kmers_index/removeNonDnaLetter.py "
            "-i {input} "
            "-o {output} "
        )
...
rule filter:
    ...
    shell:
        (
            ...
            "NR == 1 {{ print; next }} "
            "{{ key = $1; "
            "for (i = 2; i <= NF; i++) counts[key][i] += $i; }} "
            "END {{ "
            "for (key in counts) {{ "
            "printf \"%s\", key; "
            "for (i = 2; i <= NF; i++) printf \"\\t%d\", counts[key][i]; "
            "printf \"\\n\"; }} }}' "
            "{input.matrix} > {output.matrix_filtered}"
        )
rule header:
    input:
    ...
    shell:
        (
          "bash " + ribodoc_kmerIndex_tools + "kmers_index/scriptAddHeader.sh "
...
rule kamrat:
    ...
    shell:
        """
        bash {ribodoc_kmerIndex_tools}kmers_index/scriptKamrat.sh \
        {kamratImg} \
