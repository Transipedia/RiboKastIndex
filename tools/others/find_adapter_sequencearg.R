...
# Take the path, fastq name, and local path as arguments
args <- commandArgs(trailingOnly=TRUE)
fastq <- args[1]
local_path <- args[2]  # Get local path from arguments
sample <- sub(".fastq.gz", "", basename(fastq))

# Open a part of the fastq file to try finding the adapter sequence corresponding to this sequencing
try(File <- system(paste0("gzip -cd ", fastq," 2> /dev/null | dd ibs=",ibs," count=", ibs_count, " | grep -P '^[ATGC]+$'"), intern = TRUE))
File <- as.data.frame(File)

# Number of lines to check (arbitrary)
lines_check=1000
...
for(Ligne in 1 : lines_check)
{
  ...
  if(nrow(df) > 0) {
    ...
    if(Ligne > (lines_check * 0.5) & (max(df$Freq)/Ligne) > fraction_threshold)
    {
      ...
      write(Pattern2, paste0(local_path, "adapter_lists/", sample, ".txt"), append = TRUE)
      break
    }
  }
}
